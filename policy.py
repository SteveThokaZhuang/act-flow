...
class DiffusionPolicy(nn.Module):
    ...
    # flow in call abandoned
    # flow judge out
    def __call__(self, qpos, image, flow=0, actions=None, is_pad=None):
        B = qpos.shape[0]
        if actions is not None: # training time
            ...
            for cam_id in range(len(self.camera_names)):
                ...
                cam_flow = flow[:, cam_id]
                cam_input = torch.cat([cam_image, cam_flow], dim=1)
                cam_features = nets['policy']['backbones'][cam_id](cam_image, cam_flow)
            ...
            # # flow flatten
           # flatten ALL of your cameras’ flow into one [B, F] vector
            flow_flat = flow.contiguous().view(flow.shape[0], -1)        # shape = [B, num_cam*2*H*W]

            # now append it to your obs_cond
            obs_cond = torch.cat([obs_cond, flow_flat], dim=1)
            ...
            all_features = []
            print(self.camera_names) # 有几个
            for cam_id in range(len(self.camera_names)):
                cam_image = image[:, cam_id]
                cam_flow = flow[:, cam_id]
                cam_input = torch.cat([cam_image, cam_flow], dim=1) #merge?
                cam_features = nets['policy']['backbones'][cam_id](cam_image) #add flow
                # flow_backbones or cam_image内加flow
                pool_features = nets['policy']['pools'][cam_id](cam_features)
...
class ACTPolicy(nn.Module):
    ...
    def __call__(self, qpos, image, flow=0, actions=None, is_pad=None, vq_sample=None):
        env_state = None
        ...
        # flatten flow flw
        B = flow.shape[0]
        flow_flat = flow.contiguous().view(B, -1)
        env_state = flow_flat.clone()
        if actions is not None: # training time
