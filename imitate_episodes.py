# # at the very top of your script, before any mujoco imports
# import os
...
cuda_path = "/usr/local/cuda-11.8/lib64"  # 或11.7
os.environ['LD_LIBRARY_PATH'] = ":".join([
    cuda_path,
    "/usr/lib/nvidia",          # NVIDIA驱动库
    "/usr/local/lib",           # 本地安装库
...
#         init_method='env://',
#         rank=0,       # 单卡时rank固定为0
#         world_size=2  # 只有2个GPU
#     )
#     torch.cuda.set_device(0)  # 指定使用第0张卡
...
def main(args):
    ...
    enhance_flow = args['enhance_flow']
    ...
    if not is_eval:
        wandb.init(project="act-plus-plus", reinit=True, entity="Tohkasensei-uci", name=expr_name,settings=wandb.Settings(start_method="fork"))
...
# flow
def get_flow(ts, camera_names):
    flow = []
    for cam_name in camera_names:
        curr_flow = ts.observation['flows'][cam_name]
        curr_flow = rearrange(curr_flow, 'h w c -> c h w')
        flow.append(curr_flow)
    flow = np.stack(flow, axis=0)
    flow = torch.from_numpy(flow / 255).float().cuda().unsqueeze(0) #255 or not
    return flow
# cv2存一下
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    for rollout_id in range(num_rollouts):
        ...
        with torch.inference_mode():
            ...
            for t in range(max_timesteps):
                ...
                if t % query_frequency == 0:
                    ...
                    if config['enhance_flow']:
                        curr_flow = get_flow(ts, camera_names)
                ...
                if t == 0:
                    ...
                    for _ in range(10):
                        # if policy == 'ACT':
                        #     policy(qpos, curr_image, curr_flow)
                        # else:
                        #     policy(qpos, curr_image, curr_flow)
                        if config['enhance_flow']:
                            policy(qpos, curr_image, curr_flow)
                        else:
                            policy(qpos, curr_image, 0)
                ...
                if config['policy_class'] == "ACT":
                    if t % query_frequency == 0:
                        if vq:
                            ...
                            vq_sample = latent_model.generate(1, temperature=1, x=None)
                            if config['enhance_flow']:
                                all_actions = policy(qpos, curr_image, curr_flow, vq_sample=vq_sample)
                            else:
                                all_actions = policy(qpos, curr_image, 0, vq_sample=vq_sample)
                        else:
                            # e()
                            if config['enhance_flow']:
                                all_actions = policy(qpos, curr_image, curr_flow)
                            else:
                                all_actions = policy(qpos, 0, curr_flow)
                    ...
                    if t % query_frequency == 0:
                        if config['enhance_flow']:
                            ...
                            all_actions = policy(qpos, curr_image, 0)
                    ...
                    if config['enhance_flow']:
                        raw_action = policy(qpos, curr_image, curr_flow)
                    else:
                        raw_action = policy(qpos, curr_image, 0)
        ...
        episode_highest_reward = np.max(valid_rewards) if valid_rewards else 0
...
def forward_pass(data, policy, config):
    if config['enhance_flow']:
        ...
        return policy(qpos_data, image_data, flow_data, action_data, is_pad)
    else:
        image_data, qpos_data, action_data, is_pad = data
        image_data, qpos_data, action_data, is_pad = image_data.cuda(), qpos_data.cuda(), action_data.cuda(), is_pad.cuda()
        return policy(qpos_data, image_data, 0, action_data, is_pad)
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    if config['load_pretrain']:
        loading_status = policy.deserialize(torch.load(os.path.join('/home/zfu/interbotix_ws/src/act/ckpts/pretrain_all', 'policy_step_50000_seed_0.ckpt'))) # pretrain
        print(f'loaded! {loading_status}')
    if config['resume_ckpt_path'] is not None:
        loading_status = policy.deserialize(torch.load(config['resume_ckpt_path']))
        print(f'Resume policy from: {config["resume_ckpt_path"]}, Status: {loading_status}')
    policy.cuda()
    # pytorch 2.0 +jtt2.0 flw
    ...
    for step in tqdm(range(num_steps+1), desc='Train'):
        # validation
        if step % validate_every == 0:
            ...
            with torch.inference_mode():
                ...
                for batch_idx, data in enumerate(val_dataloader):
                    forward_dict = forward_pass(data, policy, config) # TODO?
        ...
        if config['enhance_flow']:
            img, qpos, actions, is_pad, flows = data
            ...
            with autocast():
                loss_dict = policy(qpos, img, flows, actions, is_pad)
                loss = loss_dict['loss']
        else:
            img, qpos, actions, is_pad = data
            img = img.cuda(non_blocking=True)
            qpos = qpos.cuda(non_blocking=True)
            actions = actions.cuda(non_blocking=True)
            is_pad = is_pad.cuda(non_blocking=True)
            # flows = flows.cuda(non_blocking=True)
            with autocast():
                loss_dict = policy(qpos, img, None, actions, is_pad)
                loss = loss_dict['loss']
...
if __name__ == '__main__':
    ...
    # for flow flw
    parser.add_argument('--enhance_flow', action='store_true', required=True)
